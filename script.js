...
function setDirectorStat(id, value) {
  const el = document.getElementById(id);
  if (el) el.textContent = String(value ?? "—");
}

function updateDirectorDashboardCounts(partial = {}) {
  Object.entries(partial).forEach(([key, value]) => setDirectorStat(key, value));
}

function openDirectorTab(tabName) {
...
async function loadPeople() {
  ...
  updateDirectorDashboardCounts({ directorStatPeople: (data || []).length });
}
...
async function loadSites() {
  ...
  updateDirectorDashboardCounts({ directorStatSites: (data || []).length });
}
...
async function loadAssets() {
  ...
  updateDirectorDashboardCounts({ directorStatAssets: (data || []).length });
}
...
async function loadMaterials() {
  ...
  updateDirectorDashboardCounts({ directorStatMaterials: (data || []).length });
}
...
async function loadReports() {
  ...
  const defectReports = directorReportsCache.filter(hasDefectData);
  ...
  updateDirectorDashboardCounts({ directorStatReports: dailyReports.length, directorStatDefects: defectReports.length });
  renderDefectsList();
  renderExportPanel();
}
...
function bindEvents() {
  ...
  if ($("#applySmartExportBtn")) $("#applySmartExportBtn").addEventListener("click", () => window.applySmartExportFilters && window.applySmartExportFilters());
  if ($("#clearSmartExportBtn")) $("#clearSmartExportBtn").addEventListener("click", () => window.clearSmartExportFilters && window.clearSmartExportFilters());
  $$('[data-quick-tab]').forEach(btn => btn.addEventListener('click', () => openDirectorTab(btn.dataset.quickTab)));
